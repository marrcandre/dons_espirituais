...
## 7. Experiência Institucional

Comparação entre a maturidade institucional dos dois projetos.

| Aspecto | Dons (atual) | Cinco Ministérios (referência) |
|---------|-------------|-------------------------------|
| Página Sobre | ❌ Não existe | ✅ `AboutView.vue` completa |
| Header institucional | ⚠️ `AppHeader.vue` básico | ✅ Logo, navegação, links |
| Footer institucional | ❌ Não existe | ✅ `AppFooter.vue` completo |
| Informações do projeto | ❌ Não existe | ✅ Objetivo, metodologia, privacidade |
| Informações do autor | ❌ Não existe | ✅ Nome, contatos, GitHub, LinkedIn |
| Outros projetos | ❌ Não existe | ✅ "Cinco Ministérios" + "Dons Espirituais" |
| Como contribuir | ❌ Não existe | ✅ Licença, GitHub |
| Licença | ❌ Não existe | ✅ Especificada |
| FAQ | ❌ Não existe | ❌ Não implementado |
| Logo / identidade visual | ⚠️ Não oficial | ✅ `AppLogo.vue` componente reutilizável |
| SEO | ❌ Não existe | ✅ Meta tags, Open Graph |
| Open Graph | ❌ Não existe | ✅ Imagem e metadados para compartilhamento |
| Sitemap | ❌ Não existe | ✅ `sitemap.xml` |
| Robots | ❌ Não existe | ✅ `robots.txt` |
| Manifest / PWA | ❌ Não existe | ✅ `manifest.json` (avaliar se faz sentido) |

### Benefícios

- **Usuários:** Entendem o propósito do projeto, quem criou, como contribuir
- **Novos desenvolvedores:** Encontram contexto, licença, diretrizes de contribuição
- **Compartilhamento:** Open Graph enriquece links compartilhados em redes sociais
- **Credibilidade:** Página Sobre, footer e identidade visual transmitem seriedade
- **SEO:** Melhora o ranqueamento em buscadores

### Prioridade

**Média** — Deve acontecer após a consolidação arquitetural (Fase 2).

A experiência institucional agrega valor visível ao usuário, mas depende de uma base arquitetural estável para ser implementada com qualidade e sem retrabalho.

### Recomendação

Incorporar como **Fase 3** do roadmap, com 3 sprints dedicadas:

| Sprint | Foco | Entregas |
|--------|------|----------|
| 11 | Identidade Institucional | Página Sobre, autor, licença, contribuição |
| 12 | Layout Institucional | Header definitivo, Footer, navegação |
| 13 | SEO e Publicação | Open Graph, Sitemap, Robots, Meta tags |
...
Além disso, o projeto Cinco Ministérios também evoluiu significativamente na **experiência institucional** (página Sobre, footer, SEO, identidade visual), área onde o Dons ainda não iniciou. Esta será tratada como Fase 3 do roadmap, após a consolidação arquitetural.

> **Arquitetura geral:** Dons ~6.5/10 → potencial 8.5/10 com application layer + testes de camada.
>
> **Experiência institucional:** Dons ~0/10 → potencial 9/10 com Fase 3.
